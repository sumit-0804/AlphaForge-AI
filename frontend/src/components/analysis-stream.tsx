...
function reduceWf(prev: WorkflowStreamState, ev: WorkflowEvent): WorkflowStreamState {
    switch (ev.type) {
        ...
        case "debate_start":
            return {
                ...prev,
                debate: { ...initialDebateState, running: true, ticker: prev.ticker },
            };
        case "debate": {
            const seeded =
        ...
        case "recommendation":
            return {
                ...
                debate: { ...prev.debate, running: false },
            };
        case "warn":
...
function StatusIcon({ status }: { status: NodeStatus }) {
    switch (status) {
        case "running":
            return <CircleNotchIcon size={16} className="animate-spin text-primary" weight="bold" />;
        case "done":
            return <CheckCircleIcon size={16} className="text-emerald-600" weight="fill" />;
        case "error":
            return <WarningCircleIcon size={16} className="text-amber-600" weight="fill" />;
        case "skipped":
            return <MinusCircleIcon size={16} className="text-muted-foreground" />;
        default:
            return <CircleIcon size={16} className="text-muted-foreground/40" />;
    }
}
...
export function WorkflowStreamView({ state }: { state: WorkflowStreamState }) {
    ...
    return (
        <div className="space-y-4">
            ...
            {/* the committee argues live when the graph took the debate path */}
